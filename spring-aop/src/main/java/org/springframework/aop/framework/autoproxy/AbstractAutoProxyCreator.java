 ...
 *
 * <p>
 *   用AOP代理封装每个合格Bean的BeanPostProcessor实现，在调用bean本身前委托给指定的拦截器
 * </p>
 *
 ...
 *
 * <p>
 *     如果有大量bean需要用类似的代理(即委托给相同的拦截器)来包装，那么这种自动代理尤其有用。
 *     您可以向bean工厂注册一个这样的后处理器，以达到相同的效果，而不是为x目标bean注册x个重复的代理定义。
 * </p>
 *
 ...
 *
 * <p>
 *     子类可以应用任何策略来决定是否要代理bean，例如，根据类型、名称、定义细节等。
 *     它们还可以返回额外的拦截器，这些拦截器应该只应用于特定的bean实例。
 *     默认的具体实现是BeanNameAutoProxyCreator，通过bean名称列表标识要代理的bean。
 * </p>
 *
...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	 ...
	 * <p>
	 *     返回给定的bean类是否是不需要代理的基础类。
	 * 默认实现将advice、advisor和AopInfrastructureBeans作为基础类。
	 * </p>
	 ...
	 * <p>
	 *     如果这个后处理器不需要给指定bean自动代理，则子类应该覆盖此方法以返回true。
	 *     我们要避免发生循环引用，就返回false
	 * </p>
	 * @param beanClass the class of the bean
	 * @param beanName the name of the bean
	 ...
	 * <p>
	 *     对于指定的bean是否应该使用targetClas而不是接口代理
	 *  </p>
	 *  <p>
	 *     检查对应bean的"preserveTargetClass"属性
	 * </p>
	 * @param beanClass the class of the bean
	 * @param beanName the name of the bean
	 ...
	 *
	 * <p>
	 * 返回子类返回的Advisors是否已预过滤以匹配bean的目标类，允许在构建AOP调用的advisors链时跳过ClassFilter检查。
	 * 默认值为false。如果需要始终返回预过滤的Advisor，则子类可以覆盖它。
	 * </p>
	 ...
	 *
	 * <p>
	 *     确定给定bean的Advisor，包括特定的拦截器以及公共拦截器，这些都适用于Advisor接口。
	 * </p>
	 *
	 * @param beanName the name of the bean
	 * @param specificInterceptors the set of interceptors that is
	 ...
	 * <p>
	 *     对指定的bean应用advices(如AOP联盟拦截器)和advisors
	 * </p>
